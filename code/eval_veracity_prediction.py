...
def print_evaluation_results(predictions, gt_labels, num_of_classes=3):
    if num_of_classes == 3:
        ...
        labels = [label_map[e.lower()] for e in gt_labels]
        predictions = [label_map[str(e).lower().strip()] for e in predictions]
        print(classification_report(labels, predictions, target_names=target_names, digits=4))
        print(confusion_matrix(labels, predictions))
        # print(f"Weighted f1 score: {f1_metric.compute(references=labels,predictions=predictions,average='weighted')}")
        # print(f"Macro f1 score: {f1_metric.compute(references=labels,predictions=predictions,average='macro')}")
        # print(f"Micro f1 score: {f1_metric.compute(references=labels,predictions=predictions,average='micro')}")
        ...
        labels = [label_map[e.lower()] for e in gt_labels]
        predictions = [label_map[e.lower()] for e in predictions]
        print(classification_report(labels, predictions, target_names=target_names, digits=4))
        print(confusion_matrix(labels, predictions))
        print()
...
def read_claims_data(test_data: str, category: str, category_wise: bool) -> List[Dict]:
    """_summary_
...
#     ground_truth_data = read_claims_data(CONFIG["test_data"], 
#                                          CONFIG["category"],
